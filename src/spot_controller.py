...
| Back               | Exit                     |
    ...
    def get_virtual_real_deviation(self, virtual_tcp_position, real_tcp_position):
    
        deviation_vector = virtual_tcp_position - real_tcp_position
        deviation = np.linalg.norm(deviation_vector)

        #print("")
        #print("distance: ", deviation)
        #print("virtual:  ", virtual_tcp_position)
    ...
    def arm_control(self):
        ...
        virtual_tcp_position = np.array([0.75, 0, 0.4])
        virtual_old_tcp_position = np.array([0.75, 0, 0.4])
        ...
        try:
            ...
            # get robot position 
            # state mashine if updated -> send only to robot
            # if not correct update digital twin from robot get state
            # dont set to new position
            check_real_position = False      
            while True:
                ...
                if(len(data)>8):
                    ...
                    virtual_tcp_position[0] += z[0] # z -> x-Achse
                    virtual_tcp_position[1] -= x[0] # x -> y-Achse
                    virtual_tcp_position[2] += y[0] # y -> z-Achse
                    #print("arm_position: ", virtual_tcp_position)
                    #print("old_position: ", virtual_old_tcp_position)
                    if(self.mode == RobotMode.Stand):

                        if not np.array_equal(virtual_old_tcp_position, virtual_tcp_position):
                            virtual_old_tcp_position = deepcopy(virtual_tcp_position)
                            check_real_position = True

                            hand_ewrt_flat_body = geometry_pb2.Vec3(x=virtual_tcp_position[0], y=virtual_tcp_position[1], z=virtual_tcp_position[2])
                            flat_body_Q_hand = geometry_pb2.Quaternion(w=1, x=0, y=0, z=0)
                            flat_body_T_hand = geometry_pb2.SE3Pose(position=hand_ewrt_flat_body,
                                                                        rotation=flat_body_Q_hand)
                            flat_body_T_hand =  math_helpers.SE3Pose.from_obj(flat_body_T_hand) #check if this is needed
                            duration = 0.2
                            arm_command = RobotCommandBuilder.arm_pose_command(
                                    flat_body_T_hand.x, flat_body_T_hand.y, flat_body_T_hand.z, flat_body_T_hand.rot.w, flat_body_T_hand.rot.x,
                                    flat_body_T_hand.rot.y, flat_body_T_hand.rot.z, BODY_FRAME_NAME, duration)
                            
                            # Send the request
                            self._issue_robot_command(arm_command)
                                
                        else:
                            if check_real_position:
                                check_real_position = False
                                real_tcp_position= self.get_real_position(robot_state_client)
                                deviation = self.get_virtual_real_deviation(virtual_tcp_position, real_tcp_position)
                                if deviation > 0.05:
                                    print(deviation)
                                    virtual_tcp_position = deepcopy(real_tcp_position)
                                    print("deviation to high - calibrated virtual position with real position")

                    else:
                        stow = RobotCommandBuilder.arm_stow_command()
                        self._issue_robot_command(stow)
                        virtual_tcp_position = [0.75, 0, 0.4]
                        virtual_old_tcp_position = [0.75, 0, 0.4]
...
if __name__ == "__main__":
    ...
    add_ctl_args(parser)
    parser.add_argument('--username', help='username of spot', default = "admin")
    parser.add_argument('--password', help='password of spot', default = "743sqhuudhzr")
